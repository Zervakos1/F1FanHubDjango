...
def add_to_cart(request, product_id):
    ...
    if product.is_premium_only and not user_has_premium_access(request.user):
        messages.error(request, 'This item is available only for users with an active premium subscription.')
...
def checkout(request):
    ...
    if blocked_items:
        messages.error(request, 'Your cart contains premium-only items. Activate premium to continue or remove those items.')
        return redirect('cart-detail')
